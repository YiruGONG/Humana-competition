...
xgb_pred_train = xgboost_pred_train$data$prob.1
...
# stack.model = glm(hi_flag~.,family = "binomial", data = stack_train)
...
```{r}
weight = optimize(function(x) roc(y, x*lgb_pred_train+(1-x) * xgb_pred_train)$auc[1], c(0,1), maximum = T)
weight
w = weight$maximum

## test result
roc(y2,lgb_pred)
roc(y2,pred3$data$prob.1)
roc(y2, w*lgb_pred+(1-w) * pred3$data$prob.1)
```
