...
grids_full = expand.grid(max_depth = c(1,2,3),
                ...
                scale_pos_weight = sample(c(20:50),10)
                )
# grids_full = grids_full %>% filter(min_data >= min_data_in_bin)
grids = grids_full[sample(1:nrow(grids_full), 0.012*nrow(grids_full)),]
...
lgb.model = lgb.train(params = lgb.grid, data = lgb_train, num_leaves = 25,
                      ...
                      eval_freq = 20, eval = lgb.normalizedgini,
                      categorical_feature = cat_idx2, data_seed = 1)
...
10.13
 comb best_iter best_score max_depth feature_fraction lambda_l1 lambda_l2
1  195      1175  0.7500824         2            0.229  2.223544 0.8900668
  scale_pos_weight
1               30
...
write.csv(output,"2022CaseCompetition_Yiru_Gong_20221016.csv",row.names = F)
```
...
```{r}
load("tmp_best.Rdata") # for p2

train_pred <- matrix(nrow=nrow(train),ncol = 0)
test_pred <- all_ens <- matrix(nrow=nrow(test), ncol = 0)
record = list()
for (i in c(1:10)){
  lgb.grid = list(objective = "binary",
                  metric = "auc",
                  max_depth = p2$max_depth[i],
                  learning_rate = 0.01, 
                  min_sum_hessian_in_leaf = 1,
                  feature_fraction = p2$feature_fraction[i],
                  bagging_fraction = 0.9, 
                  bagging_freq = 5,
                  min_data = 100, 
                  max_bin = 50,
                  lambda_l1 = p2$lambda_l1[i],
                  lambda_l2 = p2$lambda_l2[i],
                  min_data_in_bin= 100,
                  min_gain_to_split = 10,
                  min_data_in_leaf = 30,
                  # is_unbalance = TRUE)
                  scale_pos_weight = p2$scale_pos_weight[i])
  
  lgb.model = lgb.train(params = lgb.grid, data = lgb_train, num_leaves = 25,
                      num_threads = 4 , nrounds = p2$best_iter[i],
                      eval_freq = 20, eval = lgb.normalizedgini,
                      categorical_feature = cat_idx2, data_seed = 1)
  
  lgb_train_pred = predict(lgb.model, train_lgb[,-1])
  train_pred = cbind(train_pred, lgb_train_pred)
  colnames(train_pred)[ncol(train_pred)] = paste0("round_",i)
  lgb_test_pred = predict(lgb.model,x2_lgb)
  test_pred = cbind(test_pred, lgb_test_pred)
  colnames(test_pred)[ncol(test_pred)] = paste0("round_",i)
  
  ## model ensemble
  best_ens = function(par){
    val = ( train_pred %*% par )/ sum(par) ###???
    - roc(y, as.vector(val))$auc[1]
  }
  weights = optim(rep(1/i,i), best_ens,
                  lower = rep(0,i), upper = rep(1,i))
  ens_prob = as.vector( ( test_pred %*% weights$par )/ sum(weights$par) )
  all_ens = cbind(all_ens, ens_prob)
  colnames(all_ens)[ncol(test_pred)] = paste0("round_",i)
  
  roc( y2_lgb, lgb_test_pred ) #0.7394
  record[[i]] = list( auc = roc( y2_lgb, ens_prob )$auc[1],
                      round = i,
                      weights = weights )
  print(paste("ensembled",i,record[i]$auc))
}

nround = which.max( sapply(record, function(x) x[['auc']]) )
record[[nround]]$auc

# single roc compare
single_roc = apply(test_pred,2,function(x) roc( y2_lgb, x )$auc[1])
single_roc
single_roc[which.max(single_roc)]
```
